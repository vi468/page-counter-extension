...
// Настройки из хранилища: только известные ключи, чтобы значения прошлых
// версий (например, шаг клика по иконке) не тянулись дальше.
export function normalizeSettings(settings) {
  const merged = { ...DEFAULT_SETTINGS, ...(settings ?? {}) };
  return {
    defaultScope: merged.defaultScope,
    iconClickAction: merged.iconClickAction,
  };
}
...
// ---------- Единственный путь изменения счётчиков ----------

// Мутации сериализуются очередью промисов: следующая не читает хранилище,
// пока предыдущая не записала. Очередь живёт в модуле, поэтому сериализует
// изменения внутри одного контекста (у воркера, попапа и настроек свои копии
// модуля). Сбой одной мутации не залипает очередь — хвост всегда резолвится.
let mutationQueue = Promise.resolve();

// mutateCounters(mutator) — применить намерение к актуальному массиву счётчиков.
// mutator получает массив, прочитанный из хранилища в момент записи, и возвращает
// новый массив. Если вернуть тот же массив (или не массив) — записи не происходит.
// Возвращает итоговый массив счётчиков. Про DOM и экраны модуль ничего не знает.
export function mutateCounters(mutator) {
  const run = async () => {
    const counters = await getCounters();
    const next = await mutator(counters);
    if (Array.isArray(next) && next !== counters) {
...
// Изменить один счётчик по id: updater получает актуальную запись и возвращает
// патч. Если счётчика с таким id нет — возвращается исходный массив, чтобы
// «менять нечего» не порождало лишнюю запись.
export function updateCounterIn(counters, id, updater) {
  const index = counters.findIndex((c) => c.id === id);
  if (index === -1) return counters;
  const next = counters.slice();
  next[index] = { ...counters[index], ...updater(counters[index]), updatedAt: Date.now() };
  return next;
}

// Убрать счётчик по id. Если его нет — массив не меняется (лишней записи не будет).
export function removeCounterIn(counters, id) {
  if (!counters.some((c) => c.id === id)) return counters;
  return counters.filter((c) => c.id !== id);
}

// Единственный путь изменения значения: целое, не ниже нуля.
// removeAtZero: ноль означает, что считать нечего, и счётчик удаляется. Так
// работает только стирание сочетанием клавиш — Alt+0 и Alt+↓; там нет ни списка,
// ни кнопки удаления, и убрать счётчик больше нечем. В менеджере и попапе ноль
// ставится как обычное число: у счётчика в списке есть своя кнопка удаления, и
// сброс к нулю не должен ничего терять — иначе кнопка сброса бессмысленна.
export function setCounterValueIn(counters, id, value, { removeAtZero = false } = {}) {
  const next = normalizeValue(value);
  if (removeAtZero && next === 0) return removeCounterIn(counters, id);
  return updateCounterIn(counters, id, () => ({ value: next }));
}

// Прибавление на delta.
export function incrementCounterIn(counters, id, delta) {
  const found = counters.find((c) => c.id === id);
  if (!found) return counters;
  return setCounterValueIn(counters, id, found.value + Math.abs(delta));
}

// Убавление на delta: ниже нуля счётчик не уходит, ноль остаётся нулём, если
// не передан removeAtZero (см. выше).
export function decrementCounterIn(counters, id, delta, options) {
  const found = counters.find((c) => c.id === id);
  if (!found) return counters;
  return setCounterValueIn(counters, id, found.value - Math.abs(delta), options);
}
...
export function isCounterRelevant(counter, ctx) {
  // Без контекста (попап открыт, а активной вкладки нет) релевантным не считается никто.
  if (!ctx || !isValidScope(counter?.scope)) return false;
  const { tabId, url } = ctx;
...
export async function pruneTabCounters(tabId) {
  await mutateCounters((counters) => {
    ...
    return filtered.length === counters.length ? counters : filtered;
  });
}
