...
// Единственное правило обнуления: счётчик, значение которого стало нулём,
// из памяти удаляется. Ноль означает, что считать нечего, а в списке такой
// счётчик только копится. Правило живёт здесь и применяется везде — в воркере,
// попапе и менеджере, — чтобы обнуление нельзя было развести по контекстам:
// хоткеи, кнопки «−»/«↺» и ручной ввод нуля ведут себя одинаково.
export function setCounterValueIn(counters, id, value) {
  const next = normalizeValue(value);
  if (next === 0) return removeCounterIn(counters, id);
  return updateCounterIn(counters, id, () => ({ value: next }));
}

// Прибавление на delta. Обнулением не бывает — считает через тот же путь.
...
// Убавление на delta — тем же правилом: ушли в ноль, значит удалили.
export function decrementCounterIn(counters, id, delta) {
  const found = counters.find((c) => c.id === id);
  if (!found) return counters;
  return setCounterValueIn(counters, id, found.value - Math.abs(delta));
}
