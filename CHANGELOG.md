# Changelog

Формат: [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), версионирование: [SemVer](https://semver.org/).

## [Unreleased]

### Fixed
- Контекстное меню на иконке больше не создаётся дважды за один старт. `onInstalled` и `onStartup` могут прийти в одном жизненном цикле service worker'а, две последовательности `removeAll → create` накладывались, и Chrome писал в список ошибок расширения `Cannot create item with duplicate id reset-primary` / `open-manager`. Регистрация теперь проходит ровно один раз на жизнь воркера, `create` вызывается только после колбэка `removeAll`, а `chrome.runtime.lastError` читается — ошибка не всплывает, даже если гонка случится. Поведение прежнее: меню по-прежнему восстанавливаются при старте браузера.

### Added
- Регрессионный тест `tests/context-menu.test.js` на `node --test` без внешних зависимостей: мокает `chrome`, поднимает `background.js` и проверяет, что `onInstalled` + `onStartup` создают ровно два пункта меню, каждый — после `removeAll`. На коде версии 1.0.0 тест падает (4 вызова `create` вместо 2).

## [1.0.0] — 2026-04-21

Первый стабильный релиз. Расширение используется ежедневно автором, покрывает заявленный функционал подсчёта событий на страницах с привязкой к scope.

### Added
- Контекстное меню на иконке расширения (ПКМ): «Сбросить главный счётчик» и «Открыть менеджер счётчиков». Закрывает пробел, что long-press на иконке в Chrome недоступен расширениям.

### Notes
- Версии `0.1.x` и `0.2.0` помечены как pre-release: это были итерации становления, не предназначенные для широкого использования.

## [0.2.0] — 2026-04-15

### Added
- Менеджер всех счётчиков на странице настроек. Показывает счётчики независимо от текущей вкладки, сгруппированные по scope (global / domain / url / tab). Для каждого доступны: переименование, ручной ввод значения, ±step, сброс к initial, удаление. Полезно, если случайно пере/недокликал и нужно поправить значение, находясь уже на другой странице.

## [0.1.2] — 2026-04-15

### Fixed
- Цвет цифры на badge теперь белый — на синем фоне тёмный текст был плохо читаем.

## [0.1.1] — 2026-04-15

### Fixed
- Клик по иконке в режиме `increment` на странице без главного счётчика теперь автоматически создаёт счётчик с `defaultScope` из настроек и применяет шаг. Раньше клик молча игнорировался — для первого использования приходилось открывать попап и создавать счётчик вручную.

## [0.1.0] — 2026-04-15

Первый MVP.

### Added
- Модель Counter со scope: `tab` / `domain` / `url` / `global`. Дефолт — `domain`.
- Попап со списком счётчиков, релевантных текущей странице: +/-, ручной ввод значения, переименование.
- Primary-счётчик per scope-контекст, его значение отображается в badge иконки.
- Контекстное меню счётчика: сделать главным, переименовать, задать значение, изменить initial/step, reset, удалить.
- Настройки: дефолтный scope, поведение клика ЛКМ по иконке (`+N` или открыть попап), шаг клика.
- Хоткеи `Alt+↑` / `Alt+↓` / `Alt+0` (reset).
- Экспорт / импорт счётчиков и настроек в JSON.
- Автоочистка tab-scoped счётчиков при закрытии вкладки.
- Live-обновление попапа при изменениях в storage.

### Technical
- Manifest V3, service worker как ES-module.
- Хранилище — `chrome.storage.local`.
- Без сборщика, чистый JS + HTML + CSS.

[Unreleased]: https://github.com/vi468/page-counter-extension/compare/v1.0.0...HEAD
[1.0.0]: https://github.com/vi468/page-counter-extension/compare/v0.2.0...v1.0.0
[0.2.0]: https://github.com/vi468/page-counter-extension/compare/v0.1.2...v0.2.0
[0.1.2]: https://github.com/vi468/page-counter-extension/compare/v0.1.1...v0.1.2
[0.1.1]: https://github.com/vi468/page-counter-extension/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/vi468/page-counter-extension/releases/tag/v0.1.0
